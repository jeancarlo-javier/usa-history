...
export default function Timeline({
  ...
  const eventPositions = React.useMemo(() => {
    ...
    // Sort events by year to process them in chronological order.
    const sortedEvents = [...eventsData].sort((a, b) => a.year - b.year);

    let lastPosition = -Infinity;

    sortedEvents.forEach((event) => {
      let position = ((event.year - minYear) / (maxYear - minYear)) * 100;

      // If the current event's position is too close to the last one, push it.
      if (position <= lastPosition + minSpacing) {
        position = lastPosition + minSpacing;
      }

      const finalPosition = Math.min(position, 95);
      positions[event.id] = finalPosition;
            ...
            {eventsData.map((event, index) => {
              ...
              return (
                ...
                >
                  ...
                  {event.prominent ? (
                    <div
                      className={`absolute left-1/2 transform -translate-x-1/2 text-center transition-all duration-300 group-hover:scale-105 ${
                        isEven ? "-top-24" : "top-full mt-4"
                      }`}
                    >
                      {/* Connector line for prominent events */}
                      <div
                        className={`absolute left-1/2 transform -translate-x-1/2 w-px bg-amber-400/50 ${
                          isEven
                            ? "top-full h-4"
